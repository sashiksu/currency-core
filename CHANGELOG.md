...
- **237-record dataset, source-cited.** ~155 active ISO 4217 fiat currencies plus GBX (LSE Penny Sterling convention, intentionally non-ISO — see `src/data/fiat.ts` header), all citing SIX Interbank Clearing's `list-one.xml`. Top-50 cryptocurrencies by market cap with `chain` identifiers and decimals capped at 8 (citing CoinGecko top-200). 30 historical / withdrawn currencies (eurozone predecessors, post-Soviet and ex-Yugoslav redenominations, citing SIX `list-three.xml`). Every `src/data/*.ts` file ships a citation header that CI verifies on every PR.
- **Per-currency tree-shake entries.** `import { USD } from "currency-core/currencies/USD"` pulls in only USD's literal record. 237 entries codegen'd from the dataset; CI enforces a 500-byte hard cap per entry (NFR-4) — USD measures 183 B brotlied at 63% headroom.
...
- **Interactive demo site** at <https://sashiksu.github.io/currency-core/> — live code lookup, locale-aware format / parse playground, country↔currency explorer, 237-row filterable browser table, symbol disambiguator, install-matrix tabs, six real-world use-case snippets, a CodeMirror live playground covering every runtime API method, static reference blocks for the build-time API surface (per-currency tree-shake imports + compat shims), and an FAQ.
